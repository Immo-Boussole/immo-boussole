...
class IadfranceScraper(BaseScraper):

    async def get_listings(self, search_url: str) -> List[Dict]:
        """
        Extract listings from IAD France.
        """
        ...
        if html_content:
            ...
            items = soup.find_all('div', class_=re.compile(r'c-ad|card'))
            for item in items:
                try:
                    ...
                    if not url.startswith('http'):
                        url = "https://www.iadfrance.fr" + url
                    
                    title = item.find(['h2', 'p', 'span'], class_=re.compile(r'title|text')).text.strip()
    ...
    async def get_listing_details(self, url: str) -> Dict:
        """
        Scrape details for an IAD France listing.
        """
        ...
        details["url"] = url
        details["title"] = soup.find('title').text.strip() if soup.find('title') else "Annonce IAD France"
        
        desc = soup.find('div', class_=re.compile(r'description|text'))
        ...
        ext_id = url.split('/')[-1].split('.')[0]
        details["external_id"] = f"iad_{ext_id}"
