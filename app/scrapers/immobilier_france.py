...
class ImmobilierFranceScraper(BaseScraper):

    async def get_listings(self, search_url: str) -> List[Dict]:
        """
        Extract listings from Immobilier France.
        """
        ...
        if html_content:
            ...
            items = soup.find_all(['div', 'article'], class_=re.compile(r'annonce|card'))
            for item in items:
                try:
                    ...
                    if not url.startswith('http'):
                        url = "https://www.immobilier-france.fr" + url
                    
                    title = item.find(['h2', 'h3']).text.strip() if item.find(['h2', 'h3']) else "Annonce Immobilier France"
    ...
    async def get_listing_details(self, url: str) -> Dict:
        """
        Scrape details for an Immobilier France listing.
        """
        ...
        details["url"] = url
        details["title"] = soup.find('title').text.strip() if soup.find('title') else "Annonce Immobilier France"
        ...
        ext_id = url.split('/')[-1].split('.')[0]
        details["external_id"] = f"immofr_{ext_id}"
